...
Our study aims to predict the survival rates of Titanic passengers by identifying the variables that have the greatest impact on survival. To achieve this, we begin by dividing the data into two datasets: 80% of the sample will be used for training, and the remaining 20% for testing purposes.
...
formula <- as.formula("Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked + Title + ScaleFamily + Child + Mother")

nn_model <- neuralnet(
  formula,
  data = train_df,
  hidden = c(4, 2), # two hidden layers with 4 and 2 neurons respectively
  threshold = 0.01, 
  act.fct = "logistic" 
)
# Make predictions on the test data
nn_pred <- compute(nn_model, as.matrix(test_df[, c("Pclass", "Sex", "Age", "SibSp", "Parch", "Fare", "Embarked", "Title", "ScaleFamily", "Child", "Mother")]))
...
ridge_pred_obj <- prediction(ridge_pred, test_df$Survived)
...
logistic_model <- glm(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked + Title + ScaleFamily + Child + Mother, data = train_df, family = binomial())

logistic_pred <- predict(logistic_model, newdata = test_df, type = "response")

logistic_pred_binary <- ifelse(logistic_pred > 0.5, 1, 0)
actual <- test_df$Survived
confusionMatrix(as.factor(logistic_pred_binary), as.factor(actual))
```


```{r}
# Plot ROC curve
logistic_pred_obj <- prediction(logistic_pred, test_df$Survived)
logistic_perf_obj <- performance(logistic_pred_obj, "tpr", "fpr")
plot(logistic_perf_obj, main = "ROC Curve for Logistic Regression", col = "green")

```


>>>>>>> 13f23b67b5d18e9e5f87916de47d853d210ea45b
